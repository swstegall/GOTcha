...
const BattleScreen = (props) => {
  ...
  const [moneyPT, setMoneyPT] = useState(1);
  ...
  const [opponentDeck, setOpponentDeck] = useState([    [0, "King of Tigers", "🐅", 3, 2, 4, attackDef.any, skillDef.none],
  ...
  [4, "Sus","👺", 6, 1, 2, attackDef.any, skillDef.none]]);
  const [opponentMoneyPT, setOpponentMoneyPT] = useState(1);
  ...
  const [deck, setDeck] = useState([    [0, "King of Tigers", "🐅", 3, 2, 4, attackDef.any, skillDef.none],
  ...
  useEffect(() => {
    if(!isHumanTurn) {
      ...
      for (let a = 0; a < attackers.length; ++a) {
        ...
        if (tempAttackState[attacker[1][0]][attacker[1][1]] === 1)
          continue
        ...
        for (let d = 0; d < defenders.length; ++d) {
          ...
          for (let dq = 0; dq < deadQueue.length; ++dq){
            if(deadQueue[dq][0] === defender[1][0] && deadQueue[dq][1] === defender[1][1])
              defenderDead = true
          }
        ...
        if ((deadDefenders === defenders.length && !attacked) || defenders.length <= 0)
  ...
  const setSquare = (x, y) => {
    ...
    else if ( y <= 1 && attackingSquare !== null && attackState[attackingSquare[1]][attackingSquare[0]] !== 1) {
      if ( fieldPattern[y][x] !== "" ) {
        let tempField = [...fieldPattern]
        let tempAttackState = [...attackState]
      ...
      <View style={ScreenStyle.container}>
        ...
        {(!isBattle) ? (
            ...
            <View style={styles.drawContainer}>
              <StatusBar statusBarOptions={{money, health, fieldPattern, opponentEmoji, opponentHealth}} attackOpponent={attackOpponent} />
            </View>
            <View style={styles.drawField}>
              <Battlefield fieldInformation={fieldInformation} setSquare={setSquare}/>
            </View>
            <View style={styles.drawCards}>
              <CardHand hand={hand} chooseSelectedCard={chooseSelectedCard}/>
            </View>
            <Button title={(isHumanTurn)?"Next turn":"Please Wait"} onPress={() => handleNextTurnButton()}/>
            </>
          )}
      </View>
    </>
